...
const sideItems = [
  ...
  { label: "Display Settings", icon: Palette },
  ...
  return (
    <AppLayout showSidebar={false}>
      <div className="grid lg:grid-cols-[280px,1fr]">
        <aside className="border-r border-border/60 bg-sidebar/40 p-6 min-h-[calc(100vh-4rem)] flex flex-col">
          <div className="space-y-1 pb-6 border-b border-border/60">
            ...
            <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Manage your digital forge</div>
          </div>
        ...
        <div className="p-8 lg:p-10 space-y-8 animate-fade-up max-w-[1200px]">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <section className="lg:col-span-2 panel p-6 border-l-2 border-l-primary space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  <Button variant="outline" className="border-border bg-secondary/40 justify-between text-xs uppercase tracking-[0.18em] text-muted-foreground rounded-sm h-11">
                  ...
                  <div className="flex items-center justify-between bg-secondary/40 border border-border rounded-sm px-4 h-11">
                    <span className="text-xs uppercase tracking-[0.18em] text-foreground">ENABLE 2FA AUTH</span>
                    <Switch checked={twofa} onCheckedChange={setTwofa} className="data-[state=checked]:bg-primary" />
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="panel p-6 space-y-5">
              ...
              <p className="text-sm text-foreground/80 leading-relaxed">
                Your node is currently <span className="text-primary font-mono">PUBLIC</span>. Metadata is indexed by the Forge.
              </p>
              <div className="space-y-2">
                <Button variant="outline" className="w-full border-border text-xs uppercase tracking-[0.18em] rounded-sm h-10">
                  EXPORT DATA.JSON
                </Button>
          ...
          <section className="panel p-6 space-y-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-foreground">Signal Preferences</h2>
            </div>
            <div className="grid md:grid-cols-3 gap-4">
              {[
                { label: "System Alerts", desc: "Critical forge updates and node maintenance pings.", val: alerts, set: setAlerts },
                { label: "Direct Links", desc: "Real-time alerts for incoming private comms.", val: direct, set: setDirect },
                { label: "Daily Digest", desc: "Summary of activity across followed forums.", val: digest, set: setDigest },
              ].map((s) => (
                <div key={s.label} className="bg-secondary/30 border border-border rounded-sm p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold text-foreground">{s.label}</span>
                    <Switch checked={s.val} onCheckedChange={s.set} className="data-[state=checked]:bg-primary" />
                  </div>
          ...
          <section className="panel p-6 grid lg:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-foreground">Forge Appearance</h2>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Slider defaultValue={[40]} max={100} step={5} className="[&_[role=slider]]:bg-primary [&_[role=slider]]:border-primary" />
              </div>
                  ...
                  {["COMPACT", "DEFAULT", "EDITORIAL"].map((d, i) => (
                    <button
                      ...
                      className={`py-2.5 text-[10px] uppercase tracking-[0.18em] border rounded-sm ${
                        i === 0 ? "border-primary text-primary" : "border-border text-muted-foreground hover:border-primary/40"
                      }`}
                    >
                      {d}
                    </button>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {["bg-primary-glow", "bg-primary", "bg-primary-deep", "bg-[hsl(14_50%_30%)]"].map((c, i) => (
                    <button
                      key={i}
                      onClick={() => setAccent(i)}
            ...
            <div className="bg-terminal border border-border rounded-sm overflow-hidden flex flex-col">
              ...
              <div className="p-4 font-mono text-xs space-y-1.5 flex-1 text-foreground/90">
                <div className="text-muted-foreground">// PREVIEW_MODE: <span className="text-success">Active</span></div>
                <div><span className="text-primary">root@midnight-forge:</span><span className="text-success">~$</span> update --visuals</div>
                <div className="text-muted-foreground">Applying neon-noir schemas...</div>
                <div>Density: <span className="text-primary">[################----] 80%</span></div>
                <div>Status: <span className="text-success">SYNCED</span></div>
                <div className="pt-2 flex items-center"><span className="inline-block w-2 h-3 bg-primary animate-blink" /></div>
              </div>
            </div>
          </section>
