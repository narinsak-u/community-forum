...
export function ProfileContent({ username, initialProfile }: ProfileContentProps) {
        ...
        <section className="grid md:grid-cols-[120px,1fr,auto] gap-6 items-start">
          <div className="h-28 w-28 panel overflow-hidden">
            {currentProfile?.avatar ? (
              <img src={currentProfile.avatar} alt="Avatar" className="w-full h-full object-cover" width={512} height={512} />
            ) : (
              <img src="/images/forge-avatar.jpg" alt="Node avatar" className="w-full h-full object-cover" width={512} height={512} />
            )}
          </div>
                ...
                threadsData.threads.map((c) => (
                  <div key={c.id} className="flex justify-between items-start gap-4 border-b border-border/60 pb-3">
                    <div className="space-y-1.5">
                      <Link href={`/thread/${c.slug}`} className="text-sm font-semibold text-foreground hover:text-primary transition-colors">
                        {c.title}
                      </Link>
